...
Correlation shows the linear association between two random variables, or bivariate data. In contrast, two sets of variables are cointegrated, if a linear combination of those variables has a lower order of integration. The word "integration" refers to the integrated time series or order d, denoted by I(d). Now, price, rate, yield data can all be assumed as I(1), while returns, can be assumed as I(0) series. An I(0) series is referred to as stationary, which implied that the mean and variance of the time series are finite and do not change with time. A stationary series could then also be described as mean invariant, whereby the residuals are so minimized that a regression with a linear slope could be fit. Thus, to solve for co-integration, we say:
